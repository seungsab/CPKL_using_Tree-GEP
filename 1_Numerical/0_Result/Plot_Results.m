...
for IND_fcnt_type = [2 5 6 7];
    
    switch noise_amp
        case 0
            try
                load(['AUTO_COMPOSITIONAL_GEP_GPR' num2str(IND_fcnt_type)]); RMSE3=RMSE2; RMAE3=RMAE2;
            end
            load(['AUTO_COMPOSITIONAL_GPR' num2str(IND_fcnt_type)]);
            load(['Single_Kernel_GP' num2str(IND_fcnt_type)]);
        otherwise
            temp = num2str(noise_amp);
            try
                load(['AUTO_COMPOSITIONAL_GEP_GPR' num2str(IND_fcnt_type) '_Noise_' [temp(1) temp(3:end)]]);
                RMSE3=RMSE2; RMAE3=RMAE2;
            end
            load(['AUTO_COMPOSITIONAL_GPR' num2str(IND_fcnt_type) '_Noise_' [temp(1) temp(3:end)]]);
            load(['Single_Kernel_GP' num2str(IND_fcnt_type) '_Noise_' [temp(1) temp(3:end)]]);
    end    
