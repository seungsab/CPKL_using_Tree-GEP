...
addpath([pwd '\2_Physical_models']);
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Select the types for results
% Select function type
% IND_fcnt_type = 1; % OTL circuir model (6D)
% IND_fcnt_type = 2; % Borehole model (8D)
IND_fcnt_type = 3; % Wing weight model (10D)
...
switch noise_amp
    case 0
        load(['Result3_ENG_CPK_GEP_GP' num2str(IND_fcnt_type)]);
        load(['Result2_ENG_HCPK_GP' num2str(IND_fcnt_type)]);
        load(['Result1_ENG_GP' num2str(IND_fcnt_type)]);
    otherwise
        temp = num2str(noise_amp);
        load(['Result3_ENG_CPK_GEP_GP' num2str(IND_fcnt_type) '_Noise_' [temp(1) temp(3:end)]]);
        load(['Result2_ENG_HCPK_GP' num2str(IND_fcnt_type) '_Noise_' [temp(1) temp(3:end)]]);
        load(['Result1_ENG_GP' num2str(IND_fcnt_type) '_Noise_' [temp(1) temp(3:end)]]);
end
