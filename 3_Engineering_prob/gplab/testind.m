function [indfit,indres]=testind(individual,params,dataclasses,xfile,yfile);
%TESTIND    Evaluates a GPLAB individual on a different data set.
%   [FITNESS,RESULT]=TESTIND(INDIVIDUAL,PARAMS,DATACLASSES) requests the
%   names of the test data files (input and desired output, or just input
...
%      DATACLASSES - the vector of data classes, for M3GPaccuracy
%   Output arguments:
...
if (~isempty(params.numvars)) || (params.autovars)
   ...
   for v=1:s
      ...
      varterminals{v,1}=['X',num2str(v)]; % X1, X2, ...
